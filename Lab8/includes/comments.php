        ...
        <div class="row comment-form">
            <div class="col-md-8 form">
                <?php if ($_SESSION['loggedin']): ?>
                    <form class="comment-form" action="func/ajaxmanager" method="POST">
                        <textarea name="comment-text" class="form-control" cols="80" rows="4"></textarea>
                        <input type="hidden" name="id" value="<?php echo htmlspecialchars($_SERVER['QUERY_STRING']); ?>">
                        <button type="submit" name="comment-submit" class="comment-submit btn btn-outline-success mt-2"><ion-icon name="chatbubble-outline"></ion-icon></button>
                    </form>
                <?php else: ?>
        ...
        <div class="row comments">
            <?php foreach ($comments->comments as $comment): ?>
                ...
                <div class='comment-wrapper col-md-12'>
                    ...
                    <?php
                        $theid = $comment['CID'];
                        $thereply = array_filter($replies->replies, function ($data) use ($theid) {return $data['comment_parent'] === $theid;});
                    ?>
                        ...
                        <div class='col-md-8 offset-md-1 mt-2 mb-2 comment'>
                            <div class='card'>
                                <div class='card-header' style="background: lightgray;">
                                    <a href='user.php?id=<?php echo $reply['UID']; ?>' class='comment-user-id' data-comment-user-id='<?php echo $reply['UID']; ?>'><?php $reply['user_name']; ?></a> <em>replying to </em> <?php echo $reply['response_to_user']; ?> | <?php echo $reply['date_created']; ?>
                                    <nav class="comment-thumb">
                                        <ul class="nav float-right">
                                            <li class="nav-item"><ion-icon name="thumbs-up-outline" class="<?php echo $thumbsupclass; ?>" data-review-value="2" data-review-type="thumb"></ion-icon></li>
                                            <li class="nav-item thumbs-up-val"><?php echo $thumbsup; ?></li>
